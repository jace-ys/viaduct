...
func (p *ApiProvider) Start(port string) error {
	// The mux router needs to be the last middleware item added to the negroni instance
	p.negroni.UseHandler(p.mux)

	// Create the http.Server using the negroni instance
...
// Add middlewares that will be applied to every request. Middleware handlers are executed in the order they are provided.
